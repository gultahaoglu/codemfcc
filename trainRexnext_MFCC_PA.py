# -*- coding: utf-8 -*-
"""
Created on Thu Aug 29 12:55:01 2024
...
# -*- coding: utf-8 -*-
"""
Created on Tue Aug 27 07:05:44 2024
...
import json
import shutil
from resnet1 import setup_seed, ResNet
from loss import *
...
def initParams():
    ...
    parser.add_argument("-f", "--path_to_features", type=str, help="features path",default='featuresLFCC_PA')
    parser.add_argument("-p", "--path_to_protocol", type=str, help="protocol path",default='E:\POST\DeepFakeAudio\DATASETLER\ASV2019\PA\PA\ASVspoof2019_PA_cm_protocols')
    parser.add_argument("-o", "--out_fold", type=str, help="output folder", required=False, default='models_Resnext_LFCC_PA')

    # Dataset prepare
    parser.add_argument("--feat_len", type=int, help="features length", default=750)
    parser.add_argument('--padding', type=str, default='repeat', choices=['zero', 'repeat'], help="how to pad short utterance")
    ...
    parser.add_argument('--add_loss', type=str, default="ocsoftmax",choices=["softmax", 'amsoftmax', 'ocsoftmax','samo'], help="loss for one-class training")
    ...
    parser.add_argument('--initialize_centers', type=str, default="one_hot",choices=["randomly", "evenly", "one_hot", "uniform"])
...
def train(args):
    ...
    lfcc_model=modelResNeXt.ResNeXt(num_blocks=[2, 2, 2, 2], cardinality=2, bottleneck_width=64).to(args.device)
    ...
    for epoch_num in tqdm(range(args.num_epochs)):
        ...
        for i, (lfcc, audio_fn, tags, labels) in enumerate((trainDataLoader)):
        ...
        with torch.no_grad():
            ...
            for i, (lfcc, audio_fn, tags, labels) in enumerate((valDataLoader)):
                lfcc = lfcc.unsqueeze(1).float().to(args.device)
                labels = labels.to(args.device)
                ...
                if args.add_loss == "softmax":
                    ...
                    devlossDict[args.add_loss].append(ocsoftmaxloss.item())
                     ...
                     devlossDict[monitor_loss].append(samoloss.item())    
