# -*- coding: utf-8 -*-
"""
Created on Fri Aug 16 23:28:22 2024
...
class ASVspoof2019(Dataset):
    def __init__(self, access_type, path_to_features, path_to_protocol, part='train', feature='MFCC',
        ...
        with open(protocol, 'r') as f:
            ...
            if genuine_only:
                assert self.part in ["train", "eval"]
                if self.access_type == "LA":
                    num_bonafide = {"train": 2580, "eval": 2548}
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            def the_other(train_or_dev):
                assert train_or_dev in ["train", "eval"]
                res = "dev" if train_or_dev == "train" else "train"
                return res
            with open(os.path.join(self.path_to_features+'/'+ (self.part))+ '/'+ self.feature+'_'+filename +  '.mat', 'rb') as feature_handle:
                feat_mat = scipy.io.loadmat(feature_handle)
                feat_mat=feat_mat['x']
